...
def sync_static_publish_dir(root: Path, config: dict, deploy_dir_name: str, hosting_name: str) -> Path | None:
    website_dir = root / "website"
    ...
    readme_path.write_text(
        "\n".join(
            [
                ...
                "",
                f"This folder is the ready-to-upload {hosting_name} package.",
                "",
                "How it updates:",
                ...
                f"- Build output directory: deploy/{deploy_dir_name}",
                "",
...
def sync_cloudflare_pages_publish_dir(root: Path, config: dict) -> Path | None:
    return sync_static_publish_dir(root, config, "cloudflare-pages", "Cloudflare Pages")


def sync_netlify_site_publish_dir(root: Path, config: dict) -> Path | None:
    return sync_static_publish_dir(root, config, "netlify-site", "Netlify")
...
def main() -> int:
    ...
    netlify_path = sync_netlify_site_publish_dir(root, config)
    ...
    if netlify_path:
        print(f"Netlify package synced: {netlify_path}")
    return 0
